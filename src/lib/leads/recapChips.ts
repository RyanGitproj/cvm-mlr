...
function voyageursChip(value: unknown, precision: unknown): string | null {
  // « Plus de 4 » : l'effectif approximatif saisi fait foi.
  if (value === "plus") {
    return typeof precision === "number" && Number.isFinite(precision)
      ? `${precision} voyageurs environ`
      : "Plus de 4 voyageurs";
  }
  if (typeof value !== "string" || !/^[1-9]\d*$/.test(value)) return null;
  return value === "1" ? "1 voyageur" : `${value} voyageurs`;
}
...
export function recapChips(
  ...
  const chips = [
    ...
    voyageursChip(values.nbVoyageurs, values.nbVoyageursPrecision),
